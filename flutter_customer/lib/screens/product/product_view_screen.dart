...
class _ProductViewScreenState extends State<ProductViewScreen> {
  ...
  final Set<String> _wishlistedKeys = {};
  ...
  bool get _isLoggedIn {
   ...
   Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        ...
        // <-- NEW: Pre-select the color passed from the list filter
        if (widget.color != null && product.colors.isNotEmpty) {
          final targetColor = widget.color!.trim().toLowerCase();
          final matchingIndex = product.colors.indexWhere((c) => c.colorName.trim().toLowerCase() == targetColor);
          if (matchingIndex != -1) {
            _activeColorIndex = matchingIndex;
          }
        }
   ...
   Future<void> _checkWishlistStatus() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _wishlistedKeys.clear();
        _wishlistedKeys.addAll(
            items.map((p) => '${p.id}_${p.productColorId ?? 0}'));
      });
    } catch (_) {
  ...
  // ── Try-On — builds a lightweight ProductModel from the currently
  // loaded product/active color and hands it to the virtual try-on
  // flow. `skipProfileSelection: true` matches the entry point used
  // elsewhere in the app when arriving from a specific product page.
  void _openTryOn() {
    final product = _product;
    if (product == null) return;

    final productImage = _activeColor == null
        ? product.thumbnail
        : _colorCoverUrl(_activeColor!) ?? product.thumbnail;
    final tryOnProduct = ProductModel(
      id: product.id,
      ...
      sizes: _activeVariants.map((variant) => variant.size).toList(),
      colors: _colors.map((color) => color.colorName).toList(),
 ...
 Future<void> _toggleWishlist() async {
    ...
    final activeColorId = _activeColor?.productColorId ?? 0;
    final key = '${widget.productId}_$activeColorId';
    final wasWishlisted = _wishlistedKeys.contains(key);

    // Optimistic UI update, reconciled with the backend call below.
    setState(() {
      if (wasWishlisted) {
        _wishlistedKeys.remove(key);
      } else {
        _wishlistedKeys.add(key);
      }
    });
    ...
    try {
      ok = wasWishlisted
          ? await WishlistService.removeFromWishlist(widget.productId, productColorId: activeColorId)
          : await WishlistService.addToWishlist(widget.productId, productColorId: activeColorId);
    } catch (_) {
      ok = false;
    }
  ...
  // Big gallery display — the photo/spin-viewer itself, plus a floating
  // "TRY ON" button anchored to the bottom center so it's reachable from
  // every frame of the gallery, not just a fixed spot on the page.
  Widget _bigDisplay(_GalleryItem item) {
    return Stack(
      fit: StackFit.expand,
      children: [
        if (item.kind == _GalleryKind.spin)
          Product360AutoViewer(imageUrls: item.spinUrls!)
        else
          _ZoomableNetworkImage(imageUrl: item.url!),
        Positioned(
          bottom: 14,
          left: 0,
          right: 0,
             ...
             GestureDetector(
              ...
              child: Padding(
                ...
                child: Builder(
                  builder: (context) {
                    final activeColorId = _activeColor?.productColorId ?? 0;
                    final isCurrentlyWishlisted = _wishlistedKeys.contains('${product.id}_$activeColorId');
                    
                    return Icon(
                      isCurrentlyWishlisted ? Icons.favorite : Icons.favorite_border,
                      size: 24,
                      color: isCurrentlyWishlisted
                          ? Colors.red
