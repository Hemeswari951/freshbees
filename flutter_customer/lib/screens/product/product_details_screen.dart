...
class _ProductViewScreenState extends State<ProductViewScreen> {
  ...
  ProductColorModel? get _activeColor =>
    _colors.isNotEmpty
        ? _colors[_activeColorIndex.clamp(0, _colors.length - 1)]
        : null;

  List<ProductImageModel> get _activeImages =>
    _activeColor?.images ?? [];
      

  List<ProductVariantModel> get _activeVariants =>
    _activeColor?.variants ?? [];
  ...
  List<ProductTagModel> get _tags =>
    _product?.tags ?? [];
...
return nonSpin.imageUrl;
  ...
  Widget _detailsSection(double price, double? mrp, int discountPercent) {
    ...
    return Column(
      ...
      children: [
        ...
        // ── Active / Inactive status pill — DISPLAY ONLY on admin side,
        // no toggle button (see class-level NOTE for why).
  ...
  // ── Active / Inactive status pill — DISPLAY ONLY. No toggle button:
  // the admin backend only exposes list-all-products and get-by-id, so
  // there is nothing here to call if an admin wanted to flip this. If
  // admin moderation (activate/deactivate any shop's product) is wanted
  // later, add a status-update endpoint + service method first, then
  // bring back a button here.
