...
function mapListItem(row) {
  return {
    ...
    productColorId: row.product_color_id != null ? Number(row.product_color_id) : null, // <-- ADD THIS LINE
    shopId: row.shop_id,
    ...
    sizes: row.sizes || [],
    colors: row.colors || [],
    variants: row.variants || [], // NEW: [{ size, color, stockQuantity }]
  };
}
...
async function addToWishlist(req, res) {
  try {
    ...
    // Check both body AND query so the color ID is captured regardless of how Flutter sends it
    const rawColorId = req.body?.productColorId || req.query?.productColorId;
    const productColorId = rawColorId ? Number(rawColorId) : null; 
      
    await wishlistService.addToWishlist(req.customer.customerId, productId, productColorId);
...
async function removeFromWishlist(req, res) {
  try {
    ...
    // Check both body AND query so the Dart app can send it safely in the URL
    const rawColorId = req.body?.productColorId || req.query?.productColorId;
    const productColorId = rawColorId ? Number(rawColorId) : null; 

    await wishlistService.removeFromWishlist(req.customer.customerId, productId, productColorId);
